...
def basic(landmass, path="map_temp.png", draw_cities=False, supply_center_image_path="star.png", font_name="courR08.pil"):
    pad = 10
    im = Image.new(
    ...
    draw = ImageDraw.Draw(im)    
    if draw_cities:
        star = Image.open(supply_center_image_path)
        both = Image.open(supply_center_image_path)
        cannon = Image.open(supply_center_image_path)
        anchor = Image.open(supply_center_image_path)
        sw = star.size[0]/2
        sh = star.size[1]/2
        lw = both.size[0]/2
        lh = both.size[1]/2
